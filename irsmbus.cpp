...
//! \brief  опирует данные в битах
//!
//! \param[in] ap_data_in Ц указатель на начало массива, источника данных;
//! \param[out] ap_data_out Ц указатель на начало массива, приемника данных;
//! \param[in] a_index_data_in - индекс в битах, по которому располагаютс€
//!   данные в массиве Ц источнике данных;
//! \param[out] a_index_data_out Ц индекс в битах, по которому нужно переслать
//!   данные в массив Ц приемник данных;
//! \param[in] a_size_data_bit Ц размер копируемых данных в битах.\n
//! ѕередаваемые данные бьютс€ на 3 части:
//!   - middle_part участок, занимающий число бит кратное 8; \n
//!   - first_part участок, сто€щий перед middle_part;\n
//!   - last_part участок, сто€щий после middle_part.\n
//! ≈сли a_index_data_in и a_index_data_out равны 0, то дл€ копировани€ участка
//!   middle_part используетс€ функци€ memcpyex(). ¬ остальных случа€х
//!   используютс€ маски дл€ копировани€ последовательностей битов, смещенных
...
//! \brief ѕреобразование пор€дка байтов в big_endian (от старшего
//!   к младшему), если така€ операци€ требуетс€ дл€ данной архитектуры.
//!
//! \param[in, out] ap_mess Ц указатель на начало массива, в котором следует
//!   провести такое преобразование;
//! \param[in] a_start Ц начало области преобразовани€;
//! \param[in] a_length Ц длина участка преобразовани€.
...
void irs::modbus_server_t::tick()
{
  ...
  switch(m_mode)
  {
    ...
    case read_end_mode:
    {
      ...
      // 15.08.2011  рашенинников ћ. ¬.
      // Ѕыла проверка на status_success. „то, очевидно, неверно
      } else if (m_fixed_flow.read_status() == 
        irs::hardflow::fixed_flow_t::status_error)
      {
        m_mode = read_header_mode;
...
{
  // ¬ IAR возникает предупреждение, что ap_buf не используетс€,
  // если сообщени€ выключены. ƒл€ выключени€ предупреждени€
...
{
  #ifdef NOP
  //ofstream stream("D:\\Files\\Docs\\ѕрочее\\log.txt");
