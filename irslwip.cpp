...
irs::lwip::lwip_threaded_t::lwip_threaded_t(const config_t& a_config):
...
irs::lwip::lwip_threaded_t::~lwip_threaded_t()
{
  dhcp_stop(&m_netif);
  netif_remove(&m_netif);
}

mxip_t irs::lwip::lwip_threaded_t::get_ip()
{
  mxip_t ip;
  ip.addr = m_netif.ip_addr.addr;
  return ip;
...
mxip_t irs::lwip::lwip_threaded_t::get_netmask()
{
  mxip_t netmask;
  netmask.addr = m_netif.netmask.addr;
  return netmask;
...
mxip_t irs::lwip::lwip_threaded_t::get_gateway()
{
  mxip_t gw;
  gw.addr = m_netif.gw.addr;
  return gw;
...
netif* irs::lwip::lwip_threaded_t::get_netif()
{
  return &m_netif;
}

void irs::lwip::lwip_threaded_t::tick()
{
...
void irs::lwip::lwip_threaded_t::start_dhcp()
{
  const err_t err = dhcp_start(&m_netif);
  if (err == ERR_MEM) {
    IRS_LIB_LWIP_ETH_DBG_MSG_BASE("Не хватает памяти для запуска DHCP-клиента");
...
void irs::lwip::lwip_threaded_t::lwip_tick()
{
  if (m_sys_check_timeouts_loop_timer.check()) {
    sys_check_timeouts();
  }
...
mxmac_t irs::lwip::lwip_threaded_t::st_generate_mac(irs::device_code_t a_device_code)
{
  IRS_STATIC_ASSERT(sizeof(irs::device_code_t) == 1);
  const irs_u16 device_id = irs::crc16(UID_BEGIN, UID_SIZE);
  return generate_mac(a_device_code, device_id);
}

mxmac_t irs::lwip::lwip_threaded_t::get_mac()
{
  return m_mac;
}

bool irs::lwip::lwip_threaded_t::is_dhcp_ready()
{
