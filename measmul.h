...
public:
  ...
  // Чтение значения при текущем типе измерения (статусная команда)
...
//! \brief Класс для работы с мультиметром В7-78/1 через hardflow_t
class akip_v7_78_1_t: public mxmultimeter_t
{
  typedef irs_size_t size_type;
  typedef string_t string_type;
  typedef irs::hardflow::fixed_flow_t::status_t fixed_flow_status_type;
...
//! \brief Установить режим измерения постоянного напряжения
inline void irs::akip_v7_78_1_t::set_dc()
{
  m_meas_type = DC_MEAS;
}
//! \brief Установить режим измерения переменного напряжения
inline void irs::akip_v7_78_1_t::set_ac()
{
  m_meas_type = AC_MEAS;
}
...
private:
  ...
  double *mp_value;
  irs_uarc m_ch;
  irs::timer_t m_delay_between_get_value_timer;
  const size_t m_get_value_size;
  size_t m_value_size;
  irs::timer_t m_timeout_get_value_timer;
  irs_u32 m_error_count;
  const irs_u32 m_max_error_count;
