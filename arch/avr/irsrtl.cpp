...
irs::hardflow::ethernet_rtl_t::ethernet_rtl_t():
  m_rx_buf(ETHERNET_PACKET_RX),
  m_tx_buf(ETHERNET_PACKET_TX),
  m_mac_save(mac_size),
...
void irs::hardflow::ethernet_rtl_t::rtl_interrupt()
{
...
/*irs::hardflow::ethernet_rtl_t::size_type
  irs::hardflow::ethernet_rtl_t::channel_next()
{
  size_type channel = invalid_channel;
  return channel;
}

bool irs::hardflow::ethernet_rtl_t::is_channel_exists(size_type a_channel_ident)
{
  return true;
}

irs::hardflow::ethernet_rtl_t::size_type
  irs::hardflow::ethernet_rtl_t::read(size_type a_channel_ident,
...
irs::hardflow::ethernet_rtl_t::size_type 
  irs::hardflow::ethernet_rtl_t::write(size_type a_channel_ident, 
  const irs_u8 *ap_buf, size_type a_size)  
{
  return 0;
}

irs::string irs::hardflow::ethernet_rtl_t::param(const irs::string &a_name)
{
  irs::string param;
  return param;
}

void irs::hardflow::ethernet_rtl_t::set_param(const irs::string &a_name,
  const irs::string &a_value)
{

}
*/
void irs::hardflow::ethernet_rtl_t::set_rtl_ports(irs_avr_port_t a_data_port, 
...
void irs::hardflow::ethernet_rtl_t::reset_rtl() 
{
  // Запрет прерываний
  irs_disable_interrupt();
...
bool irs::hardflow::ethernet_rtl_t::wait_dma()
{
...
irs_u8 irs::hardflow::ethernet_rtl_t::read_rtl(irs_u8 a_reg_addr)
{
...
void irs::hardflow::ethernet_rtl_t::write_rtl(irs_u8 a_reg_addr, 
  irs_u8 a_reg_data)
{
  #ifdef RTL_DISABLE_INT_BYTE
  irs_disable_interrupt();
...
void irs::hardflow::ethernet_rtl_t::overrun() 
{
...
void irs::hardflow::ethernet_rtl_t::recv_packet() 
{
  write_rtl(cr, 0x1a);
  irs_u8 byte = read_rtl(rdmaport);
  byte = read_rtl(rdmaport);
  byte = read_rtl(rdmaport);
  irs_u16 rx_len_hard_cur = 0;
  IRS_LOBYTE(rx_len_hard_cur) = byte;
  byte = read_rtl(rdmaport);
  IRS_HIBYTE(rx_len_hard_cur) = byte;
  if (!m_rx_hard) {
    if (rx_len_hard_cur > ETHERNET_PACKET_RX) {
      for (irs_u16 i = 0; i < rx_len_hard_cur; i++) {
        byte = read_rtl(rdmaport);
      }
    } else {
      m_rx_hard = true;
      m_rx_len_hard = rx_len_hard_cur;
      for (irs_u16 i = 0; i < m_rx_len_hard; i++) {
        m_rx_buf[i] = read_rtl(rdmaport);
      }
    }
  } else {
    for (irs_u16 i = 0; i < rx_len_hard_cur; i++) {
...
void irs::hardflow::ethernet_rtl_t::send_packet(irs_u16 a_length, 
  irs_u8 *a_tx_buf) 
{
  #ifdef RTL_DISABLE_INT
  irs_disable_interrupt();
  ...
  write_rtl(rbcr0, IRS_LOBYTE(a_length));
  write_rtl(rbcr1, IRS_HIBYTE(a_length));
  write_rtl(cr, 0x12);
  for (irs_u16 i = 0; i < a_length; i++) {
    write_rtl(rdmaport, a_tx_buf[i]);
  }
  ...
  write_rtl(tbcr0, IRS_LOBYTE(a_length));
  write_rtl(tbcr1, IRS_HIBYTE(a_length));
  write_rtl(cr, 0x24);
...
void irs::hardflow::ethernet_rtl_t::init_rtl(const irs_u8 *ap_mac)
{
...
bool irs::hardflow::ethernet_rtl_t::get_rx_hard()
{
  return m_rx_hard;
}

bool irs::hardflow::ethernet_rtl_t::get_tx_hard()
{
  return false;
}

irs_u8* irs::hardflow::ethernet_rtl_t::get_rx_buf()
{
  return m_rx_buf.data();
}

irs_u8* irs::hardflow::ethernet_rtl_t::get_tx_buf()
{
  return m_tx_buf.data();
}

irs_u16 irs::hardflow::ethernet_rtl_t::get_rx_len_hard()
{
  return m_rx_len_hard;
}

/*void irs::hardflow::ethernet_rtl_t::tick()
{ 
