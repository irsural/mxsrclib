...
irs::avr::eeprom_t::eeprom_t(irs_uarc a_start_index, irs_uarc a_size):
  ...
  m_crc_error(false)
{
  // !!! ¬ crc32_table использовалось преобразование из __eeprom указател€
  //  в не __eeprom. “ак делать нельз€!
  // !!! ¬ crc32_table передавалс€ размер равный 0
  ...
  if (crc == *(__eeprom irs_u32*)&mp_ee_data[m_start_index]) {
    ...
    m_crc_error = true;
    #ifdef NOP
    // «ануление EEPROM при ошибке убираем как вредное
  ...
  if (a_eeprom_id != m_old_eeprom_id) {
      ...
      &is_pos_changed))
    {
      ...
      if (ap_old_eeprom_data != IRS_NULL) {
        ...
        // создаем новый EEPROM со значени€ми по умолчанию
        m_error = true;
      }
    }
