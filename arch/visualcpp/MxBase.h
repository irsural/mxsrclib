...
public:
  ...
  __fastcall TPointer::TPointer() { FType = dtUndef; FCompConv = ccReal; Method = NULL; }
  TPointer &__fastcall operator = (const TPointer &P) { Method = P.Method; FType = P.FType; return *this; }
  __fastcall TPointer::TPointer(signed char *Value) { Char = Value; FType = dtChar; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(signed short *Value) { Short = Value; FType = dtShort; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(signed int *Value) { Int = Value; FType = dtInt; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(signed long *Value) { Long = Value; FType = dtLong; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(unsigned char *Value) { UChar = Value; FType = dtUChar; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(unsigned short *Value) { UShort = Value; FType = dtUShort; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(unsigned int *Value) { UInt = Value; FType = dtUInt; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(unsigned long *Value) { ULong = Value; FType = dtULong; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(signed __int64 *Value) { Int64 = Value; FType = dtInt64; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(unsigned __int64 *Value) { UInt64 = Value; FType = dtUInt64; FCompConv = ccReal; }
	__fastcall TPointer::TPointer(TClassDblFunc Value) { Method = Value; FType = dtMethod; FCompConv = ccReal; }
	__fastcall TPointer::TPointer(TDblFunc Value) { Function = Value; FType = dtFunction; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(cmp *Value) { Complex = Value; FType = dtComplex; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(float *Value) { Float = Value; FType = dtFloat; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(double *Value) { Double = Value; FType = dtDouble; FCompConv = ccReal; }
  __fastcall TPointer::TPointer(long double* Value) { LongDouble = Value; FType = dtLongDouble; FCompConv = ccReal; }
