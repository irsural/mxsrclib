...
int MinExp = numeric_limits<double>::min_exponent10;
double DblError = 1e-9;
	...
	:TComponent(AOwner)
{
  for(int i=0; i<2; i++)
  {
    FFunc[i] = DefFunc; FAutoScale[i] = true;
  }
  FStep = 1;
  NeedAutoScale = NeedCalculate = true;
  FHide = true;
  FScaleX = FScaleY = 1;
  FPen = new TPen();
  FMarkerPen = new TPen();
...
TPoint __fastcall TMxChartItem::ConvCoor(TDblPoint P)
{
  ...
  try
  {
    ...
    y = BoundsRect.Bottom - (Height - 1)*k - 1;
  }
  catch (Exception &e)
...
{
  TPoint P, OldP; TDblPoint DP, OldDP;
  double Begin, End;
...
double TMxChartItem::Func(int i, double x) { return FData[i][int(x + 0.5)]; }
//---------------------------------------------------------------------------
...
{
  return FBoundsRect.Right - FBoundsRect.Left >= 1 && FBoundsRect.Bottom - FBoundsRect.Top >= 1;
}
//---------------------------------------------------------------------------
...
void __fastcall TMxChart::Constructor()
{
  ...
  FAutoScaleX = FAutoScaleY = true;
  FMinWidthGrid = FMinHeightGrid = 1;
  FMinWidthAuxGrid = FMinHeightAuxGrid = 0.2;
  NeedCalculate = NeedRepaint = NeedAutoScale = true;
  FArea = DblRect(-1, 1, -1, 1);
}
//---------------------------------------------------------------------------
// Конструктор компонета
...
String __fastcall TMxChart::SFF(double x, double Step)
{
  ...
  int ex = (x == 0)?0:floor(log10(fabs(x)));
  int es = (Step == 0)?0:floor(log10(fabs(Step)));
  String NumIntoStr;
  if (ex >= 0 && es >= 0 && es < 15)
  {
  ...
  int MinSize, bool IsWidth, bool AccountText)
{
  ...
  double MinSW;
  double AS = End - Begin;
  int e = (AS == 0)?0:floor(log10(fabs(AS)));
  i0 = e - 1; j0 = 0;
        ...
        if (IsWidth)
        {
          ...
          if (X0 <= End)
          {
            ...
            if (TW1 > (X0 - FArea.Left)/AS*double(Size - 1))
              TW1 = 2*TW1 - (X0 - FArea.Left)/AS*double(Size - 1);
          }
          else return Inf;
          ...
          for (double i = X0 + CurSS; i <= End; i += CurSS)
          {
            ...
            if (i + CurSS > End)
              if (TW2 > (End - i)/AS*double(Size - 1))
                TW2 = 2*TW2 - (End - i)/AS*double(Size - 1);
            int CurW = TW1 + TW2 + S;
            STW = max(CurW, STW);
            TW1 = TW2;
          }
          if (SingleLine) if (CurCanvas->TextWidth(SFF(X0, CurSS)) > Size) return Inf;
        }
        else
        {
...
void __fastcall TMxChart::PaintGrid(bool BlackAuxGrid)
{
  ...
  if (ApplyTextY)
  {
    ...
    {
      int TPY = FGridRect.Bottom - (i - FArea.Bottom)/AH*double(GH - 1) - TH/2 - 1;
  ...
  if (ApplyTextX)
  {
    ...
    for (double i = X0; i < FArea.Right; i += SW)
    {
      ...
      int TPX = (i - FArea.Left)/AW*double(GW - 1) + FGridRect.Left - TW/2;
  ...
  if (FShowAuxGrid)
  {
    ...
    if (ApplyTextY)
      SHA = StepCalc(0, SH, SH/AH*double(GH - 1), PMinHA, false, false);

    // Вычисление SWA - шага вспомогательной сетки по горизонтали
    if (ApplyTextX)
      SWA = StepCalc(0, SW, SW/AW*double(GW - 1), PMinWA, true, false);
...
{
  if (FShowBounds != Value) { FShowBounds = Value; NeedCalculate = NeedRepaint = true; }
}
//---------------------------------------------------------------------------
void __fastcall TMxChart::SetShowGrid(bool Value)
{
  if (FShowGrid != Value) { FShowGrid = Value; NeedCalculate = NeedRepaint = true; }
}
//---------------------------------------------------------------------------
void __fastcall TMxChart::SetShowAuxGrid(bool Value)
{
  if (FShowAuxGrid != Value) { FShowAuxGrid = Value; NeedCalculate = NeedRepaint = true; }
}
//---------------------------------------------------------------------------
  ...
  if (FAutoScaleX)
  {
    ...
    if (FCount && !FItems[FBaseItem]->Hide)
    {
      ...
      for (int i = 0; i < FCount; i++)
        ...
        {
          if (FArea.Left > FItems[i]->Area.Left) FArea.Left = FItems[i]->Area.Left;
          if (FArea.Right < FItems[i]->Area.Right) FArea.Right = FItems[i]->Area.Right;
        }
    }
  }
  if (FAutoScaleY)
  {
    ...
    if (FCount && !FItems[FBaseItem]->Hide)
    {
      ...
      for (int i = 0; i < FCount; i++)
        ...
        {
          if (FArea.Bottom > FItems[i]->Area.Bottom) FArea.Bottom = FItems[i]->Area.Bottom;
          if (FArea.Top < FItems[i]->Area.Top) FArea.Top = FItems[i]->Area.Top;
        }
    }
  }
...
int __fastcall TMxChart::ConvCoorX(double b, double e, double x)
{
  ...
  #endif
  return FGridRect.Left + (Width - 1)*k;
}
//---------------------------------------------------------------------------
int __fastcall TMxChart::ConvCoorY(double b, double e, double y)
{
  ...
  #endif
  return FGridRect.Bottom - (Height - 1)*k - 1;
}
//---------------------------------------------------------------------------
...
void __fastcall TMxChartSelect::ZoomIn(double FX, double FY)
{
  ...
  TDblRect VArea;
  VArea.Left = Chart->Area.Left + (1 - FX)*(Chart->Area.Right - Chart->Area.Left)/2;
  VArea.Right = Chart->Area.Left + (1 + FX)*(Chart->Area.Right - Chart->Area.Left)/2;
  VArea.Bottom = Chart->Area.Bottom + (1 - FY)*(Chart->Area.Top - Chart->Area.Bottom)/2;
  VArea.Top = Chart->Area.Bottom + (1 + FY)*(Chart->Area.Top - Chart->Area.Bottom)/2;
  Areas.push_back(Chart->Area);
  BaseItems.push_back(Chart->BaseItem);
  SaveCompConvs();
  SaveAutoScales();
...
builder_chart_window_t(irs_u32 a_size, irs_i32 a_refresh_time_ms):
  ...
  m_size(a_size),
  m_event(*this),
  mp_form(new TChartForm(m_data, m_event, a_refresh_time_ms)),
  m_position(),
  m_chart_index(0)
{
}
...
TChartForm(const data_t &a_data, chart_event_t &a_event,
  ...
  m_chart_box(new TPaintBox(this)),
  m_chart(new TMxChart(this, m_chart_box->Canvas)),
  m_select(new TMxChartSelect(m_chart.get(), m_chart_box.get())),
  m_timer(new TTimer(this)),
  m_pause_btn(new TButton(this)),
  m_dbg_info(new TLabel(this)),
  m_fix_btn(new TButton(this)),
  m_clear_btn(new TButton(this)),
  m_pause_on_text("Пауза"),
  m_pause_off_text("Продолжить"),
  m_fix_on_text("Фиксация"),
  m_fix_off_text("Разфиксация"),
  ...
  m_time_int(0.)
{
  ...
  m_panel->Align = alTop;
  InsertControl(m_panel.get());
  m_chart_box->Align = alClient;
  m_chart_box->OnPaint = ChartPaint;
  InsertControl(m_chart_box.get());
  m_timer->Interval = m_refresh_time_ms;
  m_timer->OnTimer = TimerEvent;
  m_pause_btn->Caption = m_pause_on_text;
  m_pause_btn->OnClick = PauseBtnClick;
  m_pause_btn->Left = 10;
  m_pause_btn->Top = m_panel->Height/2 - m_pause_btn->Height/2;
  InsertControl(m_pause_btn.get());
  m_dbg_info->Left = m_panel->Left + m_panel->Width - 50;
  m_dbg_info->Top = m_panel->Height/2 - m_dbg_info->Height/2;
  m_dbg_info->Anchors >> akLeft;
  m_dbg_info->Anchors << akRight;
  InsertControl(m_dbg_info.get());
  m_fix_btn->Left = m_pause_btn->Left + m_pause_btn->Width + btn_gap;
  m_fix_btn->Top = m_panel->Height/2 - m_fix_btn->Height/2;
  m_fix_btn->Caption = m_fix_on_text;
  m_fix_btn->OnClick = FixBtnClick;
  InsertControl(m_fix_btn.get());
  m_clear_btn->Left = m_fix_btn->Left + m_fix_btn->Width + btn_gap;
  m_clear_btn->Top = m_panel->Height/2 - m_clear_btn->Height/2;
  m_clear_btn->Caption = "Очистить";
  m_clear_btn->OnClick = ClearBtnClick;
  InsertControl(m_clear_btn.get());
}
...
{
  m_chart->BoundsRect = m_chart_box->ClientRect;
}
void __fastcall irs::chart::builder_chart_window_t::TChartForm::
TimerEvent(TObject *Sender)
{
  if (!m_pause) {
    connect_data(m_chart.get(), m_data);
  }
}
void __fastcall irs::chart::builder_chart_window_t::TChartForm::
PauseBtnClick(TObject *Sender)
{
  if (m_pause) {
    ...
    m_pause = irs_true;
    m_pause_btn->Caption = m_pause_off_text;
    //m_pause_time = m_time;
    m_pause_data = m_data;
    connect_data(m_chart.get(), m_pause_data);
  }
}
void __fastcall irs::chart::builder_chart_window_t::TChartForm::
FixBtnClick(TObject *Sender)
{
  if (m_fix) {
    ...
    m_fix = irs_true;
    m_fix_btn->Caption = m_fix_off_text;
  }
}
void __fastcall irs::chart::builder_chart_window_t::TChartForm::
...
connect_data(TMxChart *a_chart, const data_t &a_data)
{
  if (m_invalidate) {
    m_invalidate = irs_false;
    a_chart->Clear();
    //const chart_func_t *time_func = a_time.func.get();
    for (data_t::const_iterator it = a_data.begin();
      it != a_data.end(); it++) {
      const chart_func_t *time_func = (*it).second.time.get();
      if (time_func->size()) {
        const chart_func_t *func = (*it).second.func.get();
        TColor color = clRed;
        if ((*it).second.index%2 == 1) color = clBlue;
        a_chart->Add(&func->fn, func->size(), color);
        a_chart->Last->DataX = &time_func->fn;
        if (!m_group_all) a_chart->NewGroupY();
      }
    }
    a_chart->Paint();
  }
