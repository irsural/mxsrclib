  ...
  int Space)
{
  ...
  if (rx == 0 && ry == 0) { Canvas->MoveTo(x1, y1); Canvas->LineTo(x1 + 1, y1); }
  else
  if (rx > ry)
  {
    ...
    for (int i=0; i<n; i++)
    {
      Canvas->MoveTo(x1 + s*i*Period, y1 + s*i*Period*k);
      Canvas->LineTo(x1 + s*(i*Period + Dash), y1 + s*(i*Period + Dash)*k);
    }
    int Frec = (m > Dash)?Dash:m;
    Canvas->MoveTo(x1 + s*n*Period, y1 + s*n*Period*k);
    Canvas->LineTo(x1 + s*(n*Period + Frec), y1 + s*(n*Period + Frec)*k);
  }
  else
  {
    ...
    for (int i=0; i<n; i++)
    {
      Canvas->MoveTo(x1 + s*i*Period*k, y1 + s*i*Period);
      Canvas->LineTo(x1 + s*(i*Period + Dash)*k, y1 + s*(i*Period + Dash));
    }
    int Frec = (m > Dash)?Dash:m;
    Canvas->MoveTo(x1 + s*n*Period*k, y1 + s*n*Period);
    Canvas->LineTo(x1 + s*(n*Period + Frec)*k, y1 + s*(n*Period + Frec));
  }
  // Рисование по точкам
  /*if (!Dash) return;
...
{
  TSize Size;
  GetTextExtentPoint32(Canvas->Handle, Text.c_str(), Text.Length(), (LPSIZE)&Size);
  return Size;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
TDblRect __fastcall DblRect(double Left, double Top, double Right, double Bottom)
{
  TDblRect Temp;
...
//---------------------------------------------------------------------------
TModalResult __fastcall ShowMsg(String Caption, String Text, TMsgDlgType DlgType,
...
{
  if (AppName != "") ShowMsg(AppName, Text, mtCustom, TMsgDlgButtons () << mbOK);
