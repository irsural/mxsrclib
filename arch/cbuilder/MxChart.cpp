...
void __fastcall TMxChartSelect::SaveAutoScales()
{
  ...
  // Возможно здесь специально используется insert
  ...
  size_type a_count)
{
  ...
  if (it != m_data.end()) {
    if (a_pos < (*it).second.func->size()) {
      // Здесь учитывается, что в массиве должен остаться пустой элемент
  ...
  m_update_param_list_timer(irs::make_cnt_s(1))
{
  ...
  //mp_param_list->InsertRow(irst("СКО/|Ymin|*100, %"),
    //irst(""), true);
  mp_param_list->InsertRow(irst("DSE"), irst(""), true);
  mp_param_list->InsertRow(irst("DSE/|Yaverage|*100, %"),
    irst(""), true);
  ...
  PointsGridSelectCell(TObject *Sender, int ACol, int ARow, bool &CanSelect)
{
  if ((mp_points_grid->Col == ACol) && (mp_points_grid->Row == ARow)) {
    ...
    // Без этого пропадает курсор, если кликнуть мышью близко к границе
    //  ячейки
...
void irs::chart::builder_chart_window_t::controls_t::update_param_list()
{
  ...
  if (index >= 0) {
    ...
    mp_param_list->Values[irst("DSE")] = sko_str;
    mp_param_list->Values[irst("DSE/|Yaverage|*100, %")] = sko_percent_str;
