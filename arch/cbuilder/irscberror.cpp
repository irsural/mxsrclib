  ...
  irs::error_code_t error_code, const char *error_type) const
{
  ...
  msg_strm << "Cуdigo de error: " << (int)error_code << endl;
  ...
  msg_strm << "Nъmero de lнnea: " << mp_error_trans->line_number() << endl;
}
  ...
  handler_type_t a_handler_type)
{
  if (m_msg_first) {
    ...
    switch (a_handler_type) {
      ...
      default: {
        throw Exception("Parбmetro handler_type incorrecto configurado irs::cbuilder::error_handler_t\nArchivo: " + String(__FILE__) + "\nLнnea: " + String(__LINE__));
      }
    }
  }
  m_msg_first = true;
  ...
  if (mp_error_trans) {
    ...
    switch (error_code) {
      case irs::ec_standard: {
        ostrstream msg_strm;
        error_out(msg_strm, error_code, "Error estбndar");
        const char* msg =
      ...
      case irs::ec_assert: {
        ostrstream msg_strm;
        error_out(msg_strm, error_code, "Afirmaciуn");
        ...
        if (spec_data) {
          if (spec_data->assert_str) {
            msg_strm << "Expresiуn de afirmaciуn: " << spec_data->assert_str;
            msg_strm << endl;
          }
          if (spec_data->message) {
            msg_strm << "Mensaje de afirmaciуn: " << spec_data->message;
            msg_strm << endl;
          }
        }
        msg_strm << '\0';
